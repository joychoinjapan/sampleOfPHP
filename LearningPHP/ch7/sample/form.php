...
function show_form($errors=array()){
    ...
    $form=new FormHelperNew($defaults);
...
function validate_form(){
    ...
    if(!array_key_exists($input['sweets'],$GLOBALS['sweets'])){
    ...
    if(count($input['main_dish'])!=2){
        ...
        // we know there are two main dishes selected, so make sure they are both valid
        if(!(array_key_exists($input['main_dish'][0],$GLOBALS['main_dished']))
            &&(array_key_exists($input['main_dish'][1],$GLOBALS['main_dished']))){
...
function process_form($input){
    // look up the full names of the sweet and the main dishes in the $GLOBALS['sweets'] and $GLOBALS['main_dishes'] arrays
